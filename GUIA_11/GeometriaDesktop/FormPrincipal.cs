...
namespace GeometriaDesktop
{
    public partial class FormPrincipal : Form
    {
        ...
        public FormPrincipal(FigurasService figurasService)
        {
        ...
        private void btnConfirmarRegistro_Click(object sender, EventArgs e)
        {
            if (rbRectangulo.Checked)
            {
                ...
                RectanguloModel rectangulo = new RectanguloModel();
                rectangulo.Ancho = ancho; rectangulo.Largo = alto;
                _figurasService.AddFigura(rectangulo);
            }
        ...
        private void btnActualizarListado_Click(object sender, EventArgs e)
        {
            ...
            foreach (FiguraModel f in lista)
            {
                ...
                if (f is RectanguloModel rectangulo)
                {
                    item = new ListViewItem($"Rectángulo #{rectangulo.Id}");
                    item.SubItems.Add($"Largo: {rectangulo.Largo}");
                    item.SubItems.Add($"Ancho: {rectangulo.Ancho}");
                }
                else if (f is CirculoModel circulo)
                {
                    item = new ListViewItem($"Circulo #{circulo.Id}");
                    item.SubItems.Add($"Radio: {circulo.Radio}");
                }
